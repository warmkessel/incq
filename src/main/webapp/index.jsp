					...
					<%  for(int x=0; x < theList.size(); x++){%>					
						<div class="col-md-6 my-4">
								<div class="d-flex">
									...
									<a href="/review.jsp?id=<%=theList.get(x).getKey().getId()%>" class="pb-3 mx-3 d-block text-dark text-decoration-none border border-left-0 border-top-0 border-right-0"><%= theList.get(x).getReviewDetails().getTitle() %></a>
										<p class="mt-1 mb-0" id="<%=JspConstants.SUMMARY%>"><a href="/review.jsp?id=<%=theList.get(x).getKey().getId()%>"><%= theList.get(x).getReviewDetails().getSummary()%></a></p>
									</div>
								</div>	
						</div>
					<%
		...
		<div class="container">
			<div
				...
				<div class="col-md-3 text-md-left mb-3 mb-md-0">
					<a href="<%=JspConstants.INDEX%>"></a><img src="assets/imgs/logo.jpg" height=100px width=100px alt="INCQ"
						class="mb-0"></a>
				</div>
